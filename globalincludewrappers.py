...
class Wrapper:
    ...
    def handle_stdio_h(self, state):
        ...
        if sys.platform == 'win32':
            wrapCFunc(state, "_fdopen", restype=FileP, argtypes=(ctypes.c_int, ctypes.c_char_p))
            state.vars["stdin"] = CWrapValue(callCFunc("_fdopen", 0, "r"), name="stdin")
            state.vars["stdout"] = CWrapValue(callCFunc("_fdopen", 1, "a"), name="stdout")
            state.vars["stderr"] = CWrapValue(callCFunc("_fdopen", 2, "a"), name="stderr")
        else:
        ...
        if sys.platform == 'win32':
            wrapCFunc(state, "_isatty", restype=ctypes.c_int, argtypes=(ctypes.c_int,))
            wrapCFunc(state, "_fileno", restype=ctypes.c_int, argtypes=(FileP,))
        else:
